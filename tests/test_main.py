...
class TestMain(unittest.TestCase):
    ...
    def test_main_function_flow(self, mock_stdout, mock_open, mock_makedirs, MockBacktester, MockStrategyFactory, MockBinanceData, mock_getenv, mock_load_dotenv):
        # Mock environment variables
        mock_getenv.side_effect = lambda x: "test_key" if x == "BINANCE_API_KEY" else "test_secret"
        ...
        MockStrategyFactory.return_value.get_strategy.return_value = mock_strategy_instance
        ...
        mock_makedirs.assert_called_once_with("backtest_logs", exist_ok=True) # Log directory created
        mock_open.assert_called_once_with(unittest.mock.ANY, "w", encoding="utf-8") # Report file opened
        mock_open.return_value.__enter__.return_value.write.assert_called_once_with("Test Report Content") # Report written
    ...
    @patch('main.os.environ.get')
    @patch('sys.stdout', new_callable=StringIO)
    def test_main_function_no_api_keys(self, mock_stdout, mock_getenv, mock_load_dotenv):
        mock_getenv.side_effect = lambda x: None # Simulate no API keys

        main()

        output = mock_stdout.getvalue()
        self.assertIn("Error: Please set BINANCE_API_KEY and BINANCE_SECRET_KEY in a .env file.", output)
