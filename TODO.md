...
- [ ] `_place_buy_order`/`_place_sell_order`에 `client.create_order(...)` 연동 (MARKET 우선)
    - [x] `TradeExecutor`에 `execution_mode` 분기 추가 (현재 LIVE 분기는 stub 로그만 처리; 실제 주문 호출은 다음 단계)
    - [ ] BUY: `quoteOrderQty` 사용, SELL: `quantity` 사용, 공통: `newOrderRespType='FULL'`
    - [ ] `newClientOrderId` 아이템포턴시 적용 (재시도 시 동일 ID)
...
- [ ] 주문 예외 처리 강화: 타임아웃/재시도(backoff), idempotency 키, 네트워크 에러 복구
    - [ ] `with_retries` 래퍼: 지수 백오프(+지터), `ORDER_RETRY`/`ORDER_TIMEOUT_SEC` 적용
    - [ ] 네트워크/서버 오류 시 상태 조회(`get_order`)로 중복체결 방지
- [ ] 주문/체결 추적: `orderId`, 상태 조회, 평균 체결가/수수료 반영, 부분체결 처리
    - [ ] `FULL` 응답 기반 평균가/체결수량/수수료 집계, 미완료 시 상태 폴링 후 집계
    - [ ] 알림/로그에 `orderId`, 평균가, 체결수량, 수수료 포함
- [ ] 슬리피지/체결 안전장치: 최대 허용 슬리피지, 최소 체결 금액, 킬스위치 환경변수
    - [ ] 사전 가드: `get_orderbook_ticker` 스프레드/추정 슬리피지 > `MAX_SLIPPAGE_BPS`면 주문 차단
    - [x] `ORDER_KILL_SWITCH=true` 시 모든 LIVE 주문 차단 및 경고 알림 (현재 구현: LIVE 분기에서 즉시 차단)
- [ ] 실시간 잔고/포지션 동기화: 체결 후 잔고 갱신, 수수료 고려한 수량 계산
    - [ ] BUY: 실제 체결 수량/평균가로 `Position` 저장, 초기 `stop_price`는 ATR 기반 산출
    - [ ] SELL: 실제 평균가/수수료 포함 PnL 계산, 포지션 제거/저장
